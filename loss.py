...
class Loss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        # mask for with obj(coord) and without obj(noobj)
        coord_mask = target_tensor[:, :, :, 4] > 0  # mask for the cells which contain objects. [n_batch, S, S] confidence가 0보다 큰 grid는 true
        noobj_mask = target_tensor[:, :, :, 4] == 0 # mask for the cells which do not contain objects. [n_batch, S, S] confidence score가 0인 grid는 true
        coord_mask = coord_mask.unsqueeze(-1).expand_as(target_tensor) # [n_batch, S, S] -> [n_batch, S, S, 1] -> [n_batch, S, S, N]  obj가 있으면 true
