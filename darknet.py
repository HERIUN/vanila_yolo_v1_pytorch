    ...
    def load_weights(self, weightfile):
        ...
        for i,layer in enumerate(self.features):
            if isinstance(layer, nn.modules.conv.Conv2d):
                ...
                bn_layer.bias.data.copy_(torch.from_numpy(weight[ptr: ptr+num_b]).view_as(bn_layer.bias.data)) # l.bias
                ptr += num_b
                bn_layer.weight.data.copy_(torch.from_numpy(weight[ptr: ptr+num_b]).view_as(bn_layer.weight.data)) # l.scale
                ptr += num_b
                bn_layer.running_mean.data.copy_(torch.from_numpy(weight[ptr: ptr+num_b]).view_as(bn_layer.running_mean.data)) # l.scale
                ptr += num_b
                bn_layer.running_var.data.copy_(torch.from_numpy(weight[ptr: ptr+num_b]).view_as(bn_layer.running_var.data)) # l.scale
                ptr += num_b
